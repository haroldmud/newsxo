          ...
          >
              ...
              News?.slice(0,5)?.map((item:any, index:number) => (
                <SwiperSlide key={index}>
                  <Link to={`/search/${index}`} className='flex relative md:h-[30rem] h-fit'>
    ...
    <div className='md:w-9/12 w-11/12 mx-auto  grid md:grid-cols-3 grid-cols-2 gap-4 mt-4'>
      {
        ...
        <Link to={`/search/${index}`}  key={index} className={`w-fit relative group overflow-hidden ${index ===1 ? 'row-span-2 h-full' : index === News.length-1 ? 'col-span-2  w-full' : ''} `}>
